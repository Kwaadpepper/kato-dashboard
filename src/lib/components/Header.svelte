<script lang="ts">
  ...
  import { onFullscreenChange, toggleFullscreen } from "$lib/utils/fullscreen";
  import { onSoundChange, toggleSound } from "$lib/utils/sounds";
  ...
  function updateCurrentTime() {
    ...
    compactTime = formatClock(d, clockConfig, {
      forceNoSeconds: true,
      locale: activeLocale,
    });
    timeZoneBadge =
      clockConfig.timeZone !== "local"
        ? getTimeZoneShortLabel(clockConfig.timeZone)
        : "";
    now = Date.now();
  }
  ...
  const themeOptions = $derived<
    Array<{ id: Theme; label: string; icon: string }>
  >([
  ...
  const countDegraded = $derived(
    probes.filter((p) => p.status === "degraded").length,
  );
  const countPaused = $derived(
    probes.filter((p) => p.status === "paused").length,
  );
  const countPending = $derived(
    probes.filter((p) => p.status === "pending").length,
  );
  const countMaintenance = $derived(
    probes.filter((p) => p.status === "maintenance").length,
  );
  ...
  const isFreshnessRed = $derived(
    connectionStatus !== "connected" || freshnessSeconds >= 60,
  );
...
>
  <div class="flex items-center gap-3 sm:gap-4 min-w-0">
    ...
    >
      ...
      {#if !compact || countPaused > 0}
        ...
        >
          <span class="w-1.5 h-1.5 rounded-full bg-slate-400" aria-hidden="true"
          ></span>
      ...
      {#if countPending > 0}
        ...
        >
          <span class="w-1.5 h-1.5 rounded-full bg-blue-500" aria-hidden="true"
          ></span>
      ...
      {#if countMaintenance > 0}
        <span
          ...
          aria-label={t("header.badgeMaintenanceAria", {
  ...
  <div class="flex items-center gap-2 sm:gap-3 shrink-0">
    ...
    >
      ...
      {#if timeZoneBadge}
        <span
          class="text-[9px] px-1 py-0.5 rounded bg-slate-800 text-slate-300 font-semibold border border-slate-700/60 shrink-0"
        >
          {timeZoneBadge}
        </span>
      {/if}
    ...
    {#if !compact || isFreshnessRed}
      <span
        class="{connectionStatus !== 'connected'
          ? 'flex'
          : 'hidden min-[480px]:flex'} font-mono text-[10px] sm:text-xs px-1.5 sm:px-2 py-0.5 rounded-md border items-center gap-1 shrink-0 {freshnessClass}"
    ...
    <button
      ...
      class="p-1 rounded-md transition-colors cursor-pointer {isSoundOn
        ...
        : 'text-[var(--kato-text-secondary)] hover:text-[var(--kato-text-primary)] hover:bg-slate-800/30'}"
      title={isSoundOn
        ? t("header.soundDisableTitle")
        : t("header.soundEnableTitle")}
      aria-label={isSoundOn
        ? t("header.soundDisableAria")
        : t("header.soundEnableAria")}
    >
      {#if isSoundOn}
    ...
    <button
      ...
      title={isFullscreenActive
        ? t("header.fullscreenExitTitle")
        : t("header.fullscreenEnterTitle")}
      aria-label={isFullscreenActive
        ? t("header.fullscreenExitAria")
        : t("header.fullscreenEnterAria")}
    >
    ...
    <div class="relative" bind:this={settingsContainer}>
      ...
      {#if isSettingsOpen}
        ...
        >
          <!-- Language section -->
          <div
            class="px-3 py-1.5 text-[10px] font-semibold uppercase tracking-wider text-[var(--kato-text-secondary)] border-b border-[var(--kato-border)] flex items-center justify-between"
          >
          ...
          {#each LOCALE_OPTIONS as opt (opt.id)}
            <button
              ...
              class="w-full flex items-center justify-between px-3 py-1.5 text-left hover:bg-slate-800/30 transition-colors cursor-pointer {activeLocale ===
              opt.id
                ? 'text-emerald-400 font-medium'
                : 'text-[var(--kato-text-primary)]'}"
            >
          ...
          <button
            type="button"
            onclick={() => void toggleFullscreen()}
            class="w-full flex items-center justify-between px-3 py-1.5 text-left hover:bg-slate-800/30 transition-colors cursor-pointer {isFullscreenActive
              ? 'text-emerald-400 font-medium'
              : 'text-[var(--kato-text-primary)]'}"
            aria-label={isFullscreenActive
              ? t("header.fullscreenExitAria")
          ...
          <div
            class="px-3 py-1.5 text-[10px] font-semibold uppercase tracking-wider text-[var(--kato-text-secondary)] border-t border-b border-[var(--kato-border)]"
          >
          ...
          {#each themeOptions as opt (opt.id)}
            <button
              ...
              class="w-full flex items-center justify-between px-3 py-1.5 text-left hover:bg-slate-800/30 transition-colors cursor-pointer {activeTheme ===
              opt.id
                ? 'text-emerald-400 font-medium'
                : 'text-[var(--kato-text-primary)]'}"
            >
          ...
          <div
            class="px-3 py-1.5 mt-1 text-[10px] font-semibold uppercase tracking-wider text-[var(--kato-text-secondary)] border-t border-b border-[var(--kato-border)] flex items-center justify-between"
          >
            <span>{t("settings.sectionMarquee")}</span>
            <span class="font-mono text-emerald-400"
              >{currentMarqueeDuration}s</span
            >
          </div>
          ...
          {#each Object.entries(MARQUEE_SPEED_PRESETS) as [key, preset] (key)}
            <button
              ...
              class="w-full flex items-center justify-between px-3 py-1.5 text-left hover:bg-slate-800/30 transition-colors cursor-pointer {currentMarqueeSpeed ===
              key
                ? 'text-emerald-400 font-medium'
                : 'text-[var(--kato-text-primary)]'}"
              title={key === "slow"
                ? t("settings.marqueeSlowDesc")
                : key === "normal"
                  ? t("settings.marqueeNormalDesc")
                  : t("settings.marqueeFastDesc")}
            >
              <div class="flex items-center gap-2">
                <span>{preset.icon}</span>
                <span
                  >{key === "slow"
                    ? t("settings.marqueeSlow")
                    : key === "normal"
                      ? t("settings.marqueeNormal")
                      : t("settings.marqueeFast")} ({preset.duration}s)</span
                >
              </div>
          ...
          <div
            class="px-3 py-2 border-t border-[var(--kato-border)] flex flex-col gap-1.5 bg-slate-950/20"
          >
            <label
              for="marquee-duration-slider"
              class="flex items-center justify-between text-[11px] text-[var(--kato-text-secondary)]"
            >
              <span>{t("settings.marqueeLimit")}</span>
              <span class="font-mono font-bold text-[var(--kato-text-primary)]"
                >{currentMarqueeDuration}s</span
              >
            </label>
            <input
            ...
            />
            <div
              class="flex justify-between text-[9px] text-[var(--kato-text-secondary)]"
            >
          ...
          <div
            class="px-3 py-1.5 text-[10px] font-semibold uppercase tracking-wider text-[var(--kato-text-secondary)] border-t border-b border-[var(--kato-border)] flex items-center justify-between"
          >
          ...
          <!-- 24h / 12h format -->
          <div
            class="px-3 py-1.5 flex items-center justify-between gap-2 text-xs"
          >
            <span class="text-[var(--kato-text-secondary)]"
              >{t("settings.clockFormat")}</span
            >
            <div
              class="flex items-center gap-1 bg-slate-900/60 p-0.5 rounded border border-[var(--kato-border)]"
            >
              <button
                type="button"
                onclick={() => setTimeFormat("24h")}
                class="px-2 py-0.5 rounded font-mono text-[11px] transition-colors cursor-pointer {clockConfig.format ===
                '24h'
          ...
          >
            <span class="text-[var(--kato-text-secondary)]"
              >{t("settings.clockSeconds")}</span
            >
            <button
              type="button"
              onclick={() => setShowSeconds(!clockConfig.showSeconds)}
              class="px-2 py-0.5 rounded text-[11px] font-mono transition-colors cursor-pointer flex items-center gap-1 border border-[var(--kato-border)] {clockConfig.showSeconds
                ? 'bg-emerald-950/60 text-emerald-300 border-emerald-700/60'
                : 'bg-slate-800/40 text-slate-400'}"
              aria-label={clockConfig.showSeconds
                ? t("settings.secondsHideAria")
                : t("settings.secondsShowAria")}
            >
          ...
          <div class="px-3 py-1.5 pb-2 flex flex-col gap-1 text-xs">
            <label
              for="timezone-select"
              class="text-[var(--kato-text-secondary)] text-[11px]"
            >
          ...
          >
            ...
            <kbd
              class="px-1.5 py-0.5 rounded bg-slate-800 border border-slate-700 text-[10px] font-mono text-[var(--kato-text-secondary)] group-hover:text-emerald-300"
            >
              ?
            </kbd>
          </button>
