<script lang="ts">
  ...
  const shortcuts = $derived([
    ...
    {
      keys: activeLocale === "fr" ? ["Début", "Fin"] : ["Home", "End"],
      desc: t("keyboardHelp.jumpFirstLast"),
    },
    {
      keys: activeLocale === "fr" ? ["Entrée", "Espace"] : ["Enter", "Space"],
      desc: t("keyboardHelp.openDetail"),
    },
    {
      keys: activeLocale === "fr" ? ["Échap"] : ["Esc"],
      desc: t("keyboardHelp.closeOrExit"),
    },
...
{#if isOpen}
  ...
  >
    ...
    >
      <!-- Header -->
      <div
        class="flex items-center justify-between pb-3 border-b border-[var(--kato-border)]"
      >
      ...
      <!-- Footer -->
      <div
        class="pt-2 border-t border-[var(--kato-border)] flex items-center justify-between text-xs text-[var(--kato-text-secondary)]"
      >
        <span
          >{t("keyboardHelp.tip", {
            key: activeLocale === "fr" ? "Échap" : "Esc",
          })}</span
        >
        <button
          type="button"
