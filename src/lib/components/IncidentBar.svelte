...
{#if queueState.displayed.length === 0 && queueState.activeCount === 0}
  ...
  >
    <span
      class="text-[var(--kato-text-secondary)] text-xs sm:text-sm font-medium flex items-center gap-2"
    >
...
{:else}
  ...
  <footer
    id="incident-bar"
    class="fixed bottom-0 left-0 right-0 z-40 bg-red-950 border-t border-red-800/60 flex flex-col text-red-200 select-none transition-[max-height] duration-200 shadow-2xl {isExpanded
      ? 'max-h-80'
      : 'h-8 sm:h-10'}"
    role="alert"
    aria-live="assertive"
  ...
  >
    ...
    >
      ...
      <button
        ...
        title={isPaused
          ? t("incidentBar.resumeMarquee")
          : t("incidentBar.pauseMarquee")}
        aria-label={isPaused
          ? t("incidentBar.resumeMarquee")
          : t("incidentBar.pauseMarquee")}
      >
      ...
      >
        <div
          class="{tvMode
            ...
            : 'flex items-center gap-4 sm:gap-6 overflow-x-auto no-scrollbar'} {isPaused
            ? 'marquee-paused'
            : ''}"
        ...
        >
          ...
          <div class="flex items-center gap-8 pr-8 shrink-0">
            {#each marqueeItems as incident, idx (`b1-${incident.id}-${idx}`)}
              <div
                class="flex items-center gap-1.5 sm:gap-2 shrink-0 text-xs sm:text-sm text-red-200 font-mono"
              >
                <span
                  class="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-red-400 shrink-0"
                ></span>
                <span
                  class="font-bold text-white truncate max-w-[140px] sm:max-w-none"
                  >{incident.probeName}</span
                >
          ...
          {#if tvMode}
            <div
              class="flex items-center gap-8 pr-8 shrink-0"
              aria-hidden="true"
            >
      ...
      {#if !tvMode}
        <button
          ...
          aria-label={isExpanded
            ? t("incidentBar.collapseAria")
            : t("incidentBar.expandAria")}
    ...
    {#if isExpanded && !tvMode}
      <div
        id="incidents-drawer-list"
        class="border-t border-red-900/60 bg-red-950/95 overflow-y-auto max-h-64 p-3 space-y-2 divide-y divide-red-900/40"
      >
        {#each incidents.filter((i) => i.resolvedAt === null) as incident (incident.id)}
          <div
            class="pt-2 first:pt-0 flex items-center justify-between gap-2 text-xs font-mono"
          >
            ...
            <div class="shrink-0 flex items-center gap-2">
              <span
                class="px-1.5 py-0.5 rounded bg-red-900/60 text-red-200 uppercase font-bold text-[10px]"
              >
                {incident.type}
              </span>
