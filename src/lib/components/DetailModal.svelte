<script lang="ts">
  ...
  const color = $derived(
    probe
      ? (STATUS_COLORS[probe.status] ?? STATUS_COLORS.up)
      : STATUS_COLORS.up,
  );
  const statusLabel = $derived(
    probe ? getStatusLabel(probe.status, activeLocale) : "",
  );
    ...
    async function fetchHistory(id: string) {
      ...
      try {
        const res = await fetch(
          `/api/probes/${encodeURIComponent(id)}/history`,
          {
  ...
  const historyEvents = $derived(
    probe ? buildProbeHistoryEvents(probe, mergedIncidents, nowTime) : [],
  );
  // 24h metric summary
  const historyStats = $derived(
    probe ? computeProbeHistoryStats(probe, mergedIncidents, nowTime) : null,
  );
...
{#if probe}
  ...
  >
    ...
    >
      ...
      <div>
        <div
          class="p-6 border-b border-slate-800 flex items-start justify-between gap-4"
        >
        ...
        <div class="p-6 space-y-6">
          ...
          <div>
            <span
              class="text-xs font-semibold text-slate-400 uppercase tracking-wider block mb-2 font-sans"
            >
            ...
            >
              <span
                class="w-2.5 h-2.5 rounded-full {color.bgClass} shadow-xs {probe.status ===
                'down'
                  ? 'animate-pulse'
                  : ''}"
              ></span>
              <span>{statusLabel}</span>
              <span class="text-xs opacity-75 font-mono uppercase"
                >({probe.status})</span
              >
            </div>
            ...
            {#if probe.status === "down" && probe.downSince}
              <div
                class="mt-3 p-3 rounded-lg bg-red-950/40 border border-red-800/50 text-xs text-red-300 flex items-center gap-2"
              >
                <Clock class="w-4 h-4 text-red-400 shrink-0" />
                <span
                  >{t("detailModal.downSince", {
                    duration: formatDuration(probe.downSince),
                  })}</span
                >
              </div>
            {/if}
          ...
          <div class="grid grid-cols-2 gap-3">
            ...
            <div
              class="p-4 rounded-xl bg-slate-800/40 border border-slate-700/50"
            >
              <div
                class="flex items-center gap-1.5 text-slate-400 text-xs mb-1.5"
              >
          ...
          <div class="grid grid-cols-2 gap-3">
            ...
            >
              ...
              <p
                class="text-sm font-semibold font-mono uppercase text-slate-200"
              >
                {probe.criticality}
              </p>
            </div>
          ...
          <div class="space-y-3 pt-2">
            <div class="flex items-center justify-between">
              <span
                class="text-xs font-semibold text-slate-400 uppercase tracking-wider flex items-center gap-1.5 font-sans"
              >
              ...
              <div class="flex items-center gap-2">
                ...
                {/if}
                <span
                  class="text-[11px] font-mono px-2 py-0.5 rounded border {historyStats &&
            ...
            <div
              class="p-3.5 rounded-xl bg-slate-800/40 border border-slate-700/50 space-y-2.5"
            >
              <div
                class="flex items-center justify-between text-[11px] font-mono text-slate-400"
              >
                <span>{t("detailModal.twentyFourHoursAgo")}</span>
                <span class="text-emerald-400 font-semibold">
                  {historyStats
                    ? t("detailModal.availablePercent", {
                        percent: historyStats.availabilityPercentage.toFixed(2),
                      })
                    : "—"}
                </span>
              ...
              <div
                class="flex items-center gap-1 h-6 w-full py-0.5 select-none"
                role="img"
                aria-label={t("detailModal.hourlyAvailabilityAria")}
              >
                {#each slots as slot (slot.index)}
                  <div
                    class="flex-1 h-full rounded-[1.5px] transition-transform duration-100 hover:scale-y-125 cursor-pointer {getSlotBgClass(
                      slot.status,
                    )}"
                    title={slot.label}
                  ></div>
              ...
              <div
                class="flex items-center justify-between text-[10px] text-slate-400 font-mono pt-1 border-t border-slate-800/80"
              >
            ...
            <div class="grid grid-cols-3 gap-2 text-center text-xs font-mono">
              <div
                class="p-2.5 rounded-lg bg-slate-800/30 border border-slate-700/40"
              >
                <span
                  class="text-[10px] uppercase text-slate-400 block font-sans"
                  >{t("detailModal.kpiIncidents")}</span
                >
                <span
                  class="font-bold text-sm {historyStats &&
                  historyStats.totalIncidents > 0
                    ? 'text-amber-400'
                    : 'text-emerald-400'}"
                >
              ...
              >
                <span
                  class="text-[10px] uppercase text-slate-400 block font-sans"
                  >{t("detailModal.kpiDowntime")}</span
                >
                <span class="font-bold text-sm text-slate-200">
                  {formatDurationCompact(
                    historyStats?.downtimeSeconds ?? 0,
                    activeLocale,
                  )}
                </span>
              </div>
              ...
              >
                <span
                  class="text-[10px] uppercase text-slate-400 block font-sans"
                  >{t("detailModal.kpiStreak")}</span
                >
                <span class="font-bold text-sm text-slate-200">
                  {formatDurationCompact(
                    historyStats?.currentStreakSeconds ?? 0,
                    activeLocale,
                  )}
                </span>
              </div>
            ...
            <div
              class="rounded-xl bg-slate-800/30 border border-slate-700/50 p-3 space-y-2.5"
            >
              <span
                class="text-[11px] font-semibold text-slate-400 uppercase tracking-wider block font-mono"
              >
              ...
              {#if historyEvents.length === 0}
                ...
                <div
                  class="flex items-center gap-3 py-2 px-1 text-xs text-slate-300"
                >
                  <div
                    class="w-7 h-7 rounded-full bg-emerald-950/60 border border-emerald-800/50 flex items-center justify-center shrink-0"
                  >
                    <CircleCheck class="w-4 h-4 text-emerald-400" />
                  </div>
                  <div class="min-w-0">
                    <p class="font-semibold text-emerald-300 text-xs">
                      {t("detailModal.noInterruption")}
                    </p>
                    <p class="text-[11px] text-slate-400">
                      {t("detailModal.continuousOperation")}
                    </p>
                  </div>
                </div>
              {:else}
                ...
                <div class="space-y-2 max-h-56 overflow-y-auto pr-1">
                  {#each historyEvents as event (event.id)}
                    <div
                      class="p-2.5 rounded-lg bg-slate-900/70 border border-slate-800 flex items-start justify-between gap-3 text-xs font-mono"
                    >
                      <div class="flex items-start gap-2.5 min-w-0 flex-1">
                        <span
                          class="w-2.5 h-2.5 rounded-full mt-1 shrink-0 {event.status ===
                          'down'
                            ? 'bg-rose-500 shadow-xs'
                            : event.status === 'degraded'
                              ? 'bg-amber-400'
                              : 'bg-emerald-400'} {event.resolvedAt === null
                            ? 'animate-ping'
                            : ''}"
                        ></span>
                        ...
                        <div class="min-w-0 flex-1">
                          <p
                            class="font-bold {event.status === 'down'
                              ? 'text-rose-300'
                              : event.status === 'degraded'
                                ? 'text-amber-300'
                                : 'text-emerald-300'}"
                          ...
                          {#if event.cause}
                            <p
                              class="text-[10px] text-slate-300 font-sans mt-1 bg-slate-800/80 px-1.5 py-0.5 rounded border border-slate-700/50 inline-block"
                            >
                              {event.cause}
                            </p>
                          {/if}
                      ...
                      <div class="shrink-0 text-right">
                        {#if event.resolvedAt === null}
                          <span
                            class="inline-block text-[10px] font-bold px-2 py-0.5 rounded bg-rose-950 text-rose-300 border border-rose-800 animate-pulse"
                          >
                            {t("detailModal.ongoingBadge", {
                        ...
                        {:else if event.duration}
                          <span
                            class="inline-block text-[10px] px-2 py-0.5 rounded bg-slate-800 text-slate-300 border border-slate-700"
                          >
                            {formatDurationCompact(
                              event.duration,
                              activeLocale,
          ...
          <!-- System metadata -->
          <div
            class="p-4 rounded-xl bg-slate-800/20 border border-slate-800 text-xs font-mono space-y-2 text-slate-400"
          >
            ...
            <div class="flex justify-between">
              ...
              <span class="text-slate-300"
                >{new Date(probe.lastCheck).toLocaleTimeString(
