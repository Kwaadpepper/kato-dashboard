<script lang="ts">
  ...
  let probes = $state<NormalizedProbe[]>(
    data.initialState?.probes ? [...data.initialState.probes] : [],
  );
  // svelte-ignore state_referenced_locally
  let incidents = $state<NormalizedIncident[]>(
    data.initialState?.incidents ? [...data.initialState.incidents] : [],
  );
  // svelte-ignore state_referenced_locally
  ...
  let previousStatuses = $state<Record<string, ProbeStatus>>(
    data.initialState?.probes
      ? Object.fromEntries(
          data.initialState.probes.map((p) => [p.id, p.status]),
        )
      : {},
  );
  ...
  const isCriticalDownRatio = $derived(
    probes.length > 0 && downCount / probes.length >= 0.3,
  );
  ...
  function setupSSE() {
    ...
    disconnectSSE = connectSSE(
      ...
      (delta: DashboardDelta) => {
        ...
        if (
          delta.newIncidents.length > 0 ||
          delta.resolvedIncidentIds.length > 0
        ) {
...
<svelte:head>
  <title
    >{currentLocale
      ? t("common.pageTitle", { count: sortedProbes.length })
      : ""}</title
  >
</svelte:head>
...
<SkipLink
  hasIncidents={incidents.filter((i) => i.resolvedAt === null).length > 0}
/>
...
<div
  ...
  class="h-screen w-screen overflow-hidden flex flex-col transition-colors duration-500 text-[var(--kato-text-primary)] select-none {isCriticalDownRatio
    ? 'bg-red-950/20'
    : 'bg-[var(--kato-bg-primary)]'} {containerFlashing
    ? 'animate-kato-border-flash'
    : ''} {isTvMode ? 'tv-mode' : ''} {isCompactHeader
    ? 'pt-8'
    : 'pt-12'} pb-8 sm:pb-10"
>
  <!-- Fixed Header -->
  ...
  {#if connectionStatus !== "connected"}
    <div
      class="fixed {isCompactHeader
        ? 'top-8'
        : 'top-12'} left-0 right-0 z-40 bg-red-600 text-white font-medium text-xs sm:text-sm py-1 px-4 flex items-center justify-center gap-2 shadow-lg backdrop-blur-xs transition-all animate-pulse"
      role="alert"
      aria-live="assertive"
    >
      <span
      ...
      <span class="text-red-100 text-xs hidden sm:inline"
        >{t("common.reconnecting")}</span
      >
    </div>
  {/if}
  ...
  <main
    ...
    class="flex-1 w-full h-full relative focus:outline-none {isTvMode
      ? 'animate-kato-drift'
      : ''} {layout.overflows
  ...
  >
    ...
    {#if isMobile && (pullDistance > 0 || isRefreshing)}
      ...
      >
        ...
        {:else if pullDistance >= 50}
          <ArrowDown
            class="w-4 h-4 text-emerald-400 rotate-180 transition-transform duration-200"
          />
          <span class="text-emerald-400 font-semibold"
            >{t("pullToRefresh.release")}</span
          >
        {:else}
          <ArrowDown
          ...
          <span class="text-slate-400"
            >{t("pullToRefresh.pull", { distance: pullDistance })}</span
          >
        {/if}
      </div>
    {/if}
    ...
    {:else}
      <div
        class="w-full h-full flex flex-col items-center justify-center text-slate-500 font-mono text-sm"
      >
        <span
          class="w-6 h-6 border-2 border-emerald-500 border-t-transparent rounded-full animate-spin mb-3"
        ></span>
        {t("common.loading")}
      </div>
    {/if}
